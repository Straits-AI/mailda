...
- **The composer is plain text**, one To field and files: no CC/BCC in the interface (the API takes both), no
  HTML, no signatures or templates, no quoted reply. Mail is never marked read or unread, and there is no
  archive, trash or spam folder — labels are the one way to sort it (ADR 13).
