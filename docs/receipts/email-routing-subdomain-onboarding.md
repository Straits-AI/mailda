...
Within a minute, live in **public DNS**, by `dig` against Cloudflare's authoritative nameserver, not the API's
...
§10's domain topology makes a **delegated operational subdomain**, `ops.example.com` or
...
subdomain and one to an **apex** control address, **never arrived**, while the external Gmail
