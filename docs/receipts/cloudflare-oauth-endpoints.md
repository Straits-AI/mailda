...
called it *"a spec conformance bug in Mailda"*. The encoding fix is correct and stays, since RFC 6749 §2.3.1 does
...
Authorization Code flow for third-party clients, no client credentials and no device flow, so a grant requires
