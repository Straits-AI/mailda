...
something awaits the import. So an inbound message, the hot path that runs on every
...
does, that the AST is derived from `source_text` and never sent beside it, the direction is settled:
...
- **`butler.yaml_max_alias_count = 100`**: the alias expansions one parse may perform.
  ...
  `budget` did not, which is true for every value, so raising the budget to 1000 left it green. It tested the
