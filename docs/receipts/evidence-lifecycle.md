...
At `n = 2` that is `4 × list_limit + 6`, which is **806** at 200, which is exactly the figure the 19 August correction
...
objects it did not list. That is a claim about the whole source tree, so a claim that had to be enforced somewhere
...
One object per manifest whatever the recipient count, since the same bytes go to every recipient, so this is linear
