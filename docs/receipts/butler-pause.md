...
**One receipt, two kinds of number, and the split is the first thing to read**, in `send-breakers.md`'s
...
**Cost if wrong, in each direction.** Too low: a chatty exchange inside an hour stops the Butler,
...
**not for want of a table**: `butler_runs` supports it in one `COUNT(*)`, which is exactly the shape #66's
