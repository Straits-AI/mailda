...
§5A begins at **install**. Steps 1 to 3 of that arc have no specification anywhere in the
...
is almost certainly a technical founder or ops-minded engineer at a 5 to 50 person company,
...
restarting. That is a good design and a rare one.

---

## Stages 5 to 13. The checklist, walked
...
it works. Every mail product a user has ever adopted showed them a working inbox in under
two minutes.

**The MX cliff at step 5 does not feel reversible.** Pointing MX at Cloudflare means live
...
ADR 25 withdrew it. Cloudflare's free plan forces **24-hour, non-configurable queue
...
that can lose mail, which is unacceptable for an evaluation that is meant to build trust.
...
pretend the problem is solved. With connectors withdrawn, the two candidates are the ones
already identified here: seed the install with fixture mail, and promote the delegated
subdomain from a §5A parenthetical to the recommended path. §10's domain topology already
defaults to `ops.example.com` or `mail.example.com`; onboarding does not yet reflect that.


---

## Resolved, 3 August 2026, by ADR 26
...
Two questions were being conflated. *"Do I want this?"* is answerable with any realistic
data, and it never needed the prospect's own mail. *"Does it work for my mail?"* is
unanswerable without pointing MX at Cloudflare, by fixtures or anything else. Two candidate
answers were burned attacking the second: provider connectors (ADR 4) and a free-plan trial
(ADR 25).
...
The second structural finding, first value at step 8 of 13, is addressed by reordering. The
inbound test is now step 6, with the five steps before it taking about a minute each. The
...
Node. That removes the lifecycle problem this document worried about: fabricated records
