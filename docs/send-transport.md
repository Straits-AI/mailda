...
`canSend`, `arbitraryRecipients` and `verifiedAt`. But only one adapter was wired, so a question the envelope
was designed to answer had the same answer for every send ever made.
...
**The binding is preferred wherever it exists**, for two reasons. It holds no credential, so a Node using it
has nothing that can leak and nothing to rotate. And it is the only adapter that can carry `authored`
fidelity, which customer mail uses because the record must prove the exact bytes.

So the REST adapter answers one question: what does a Node do when it has no binding? Before #86 the answer
was "nothing, permanently". The binding arrives by editing `wrangler.jsonc` and redeploying, which a Node
whose operator cannot redeploy it cannot do.

When neither is available the transport still returns the binding, deliberately. Its refusal names the
`send_email` binding an operator should install. The REST adapter's refusal would name a token, which is the
second-best answer. The most useful refusal wins.

## Two of the ticket's three arguments did not survive, and that shaped the adapter

#86 argued that the REST endpoint takes structured recipients in one request where the binding takes one per
call, so fifty recipients would be fifty subrequests. This tree already answers both halves, against it:
...
- Batching would also save nothing. *"Measured before choosing: one structured send to three recipients moved
...
optimistic `handed_over`. And `adapter` on the envelope now carries information.
...
test of ADR 22, *"every credential that authorizes an external effect is a **Secrets Store** binding"*. The
rule as written does not survive ADR 24. `wrangler.jsonc` already records why. A `secrets_store_secrets`
block needs an account-specific `store_id` in committed config, which is exactly the byte-identical-fork
collision ADR 24 forbids, and measurement showed that removing the id drops the binding **silently** rather
than relinking the way D1 and R2 do.

ADR 28 settled this for the encryption keys and its reasoning transfers whole. What ADR 22 was buying is
that *serializing `env` discloses nothing*, and that survives when the secret arrives over RPC and through a
...
The transport reads it with `await unwrapCredential(...)` per submission and does **not cache** it. A cached
transport token is a secret with a lifetime beyond the call that needed it, which is the property ADR 22
exists to prevent, and the saving would be one indexed read of a one-row table.

**No route returns it.** `GET /api/transport` reports *that* a token is configured, for which account, and
when. That is the whole question an operator has. A route that could hand it back would make every
administrator a holder of the account's sending authority.

## Why the surface is the interface and not the CLI

`mailda set-password` is a script because a password hash needs no vault. This cannot be a script. Wrapping
needs the KeyVault Durable Object, which only the Worker can reach, so **a credential this Node encrypts can
only be supplied through this Node.**
