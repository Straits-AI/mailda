...
| `act` | yes | changes something, and a person can undo it | 11 |
| `governed` | **no** | needs more than one person, or cannot be undone | 33 |
| `operator` | **no** | installation, credentials, maintenance | 54 |
...
**The tier is necessary and not sufficient**, and that column used to say "offered" flat. 52 routes are `read`
or `act`; **27** are offered. The difference is the second question, can a machine ever be *provisioned* for
this route, and it is asked in `authority.ts` rather than here:

| withheld by | how many | example |
|:--|--:|:--|
| the tier | 88 | `POST /api/sends`; sealing a send is the one act nobody can undo |
...
This sentence said **one** until 3 September 2026, by which point the code had eight. A document about
completeness carrying a stale count, which is the defect the tier table above exists to catch and which the
tier table could not see, because it counts tiers rather than exceptions. The full list with a reason for each
is in `test/node/agent-exposure-world.test.ts`; they fall into three kinds:

- **a page rather than a question**: `GET /index.html`.
...
The action ceiling is pinned at mint and there is deliberately no route that widens one. That is §16's *"new grants
