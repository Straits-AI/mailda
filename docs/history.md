...
What each change found on the way. These sections were the README's running account, moved here on
16 September 2026 so the README could go back to describing the product; nothing was rewritten. The
order is the order it was written in — newest last within each part.

The first part is the run of feature notes that sat under the status table; the second is the per-change
sections that followed, each headed by the issue it closed.
...
an observation, not a claim. ([receipt](./receipts/email-sending-events.md))
...
exactly one, whatever comes and goes. Full account in [`docs/butler-engine.md`](./butler-engine.md).
...
possibilities** ([receipt](./receipts/deploy-drill-live-account.md)): deploying the second Node exits 0
...
that splits its record stops being one row per act. ([receipt](./receipts/message-page-size.md))
...
- **Flow Blue cannot carry body text on Mist or Sky.** Measured: 4.53:1 on white — AA by three
  ...
  person reads. ([receipt](./receipts/contrast-tokens.md))
...
[`docs/receipts/`](./receipts/) and the rule in [`AGENTS.md`](../AGENTS.md).
...
([receipt](./receipts/authz-check-rows-read.md))
...
([receipt](./receipts/butler-run-cost.md))
...
([receipt](./receipts/message-metadata-bytes.md))
...
([receipt](./receipts/evidence-lifecycle.md))
...
trust into one you can check. ([receipt](./receipts/audit-and-log-retention.md))
...
was a broken probe is worse than no number. ([receipt](./receipts/r2-auto-provisioning.md))
...
([receipt](./receipts/doctor-check-cost.md))
...
runtime. ([receipt](./receipts/password-hash-cost.md))
...
was actually buying. See [`docs/send-transport.md`](./send-transport.md).
...
See [`docs/machine-surfaces.md`](./machine-surfaces.md).
...
[`false-claim-detectability`](./receipts/false-claim-detectability.md). Counts in prose — 261 of them,
...
([re-measured](./receipts/evidence-integrity-cost.md), and the old receipt's `stale_when` named this
...
that cannot exist, a gate comparing against the wrong thing. [`docs/disaster-recovery.md`](./disaster-recovery.md)
...
([receipt](./receipts/temporary-account-provisioning.md)):
...
([receipt](./receipts/cloudflare-oauth-node-as-client.md)). A private client was created with one
...
[`docs/cloudflare-grant.md`](./cloudflare-grant.md) carries the whole design. The state worth naming here
...
([receipt](./receipts/cloudflare-oauth-endpoints.md)).
...
[`docs/supervised-access.md`](./supervised-access.md).
...
[recorded](./receipts/contrast-tokens.md) with the numbers so the choice is legible rather than looking
...
otherwise ([receipt](./receipts/message-search-cost.md), `authz.list.max_rows_read` is 1,000):
