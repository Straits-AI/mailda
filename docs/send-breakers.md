...
|:--|:--|:--|--:|--:|
| `volume` | `send_recipients` handed over | none; it is a count | 1h | 500 recipients |
...
`send_counters` was rejected as the volume source for two reasons: it is a **maintained** cell
...
|:--|--:|--:|--:|:--|
| `evaluateBreakers`, empty / full / paused | none | **1** | 2 | `test/breaker-cost.measure.test.ts` |
| `sealManifest`, no policies | 11 | **12** | none | `test/breaker-cost.measure.test.ts` |
...
[#66][66] would have been *expressible and unusable*, and what changed is not the design but the tables under it.
...
- **A runs-per-window breaker is what is now named absent**, and *not* for want of a table. `butler_runs`
...
tables to count then, and the domain pause is a human act that needs no Butler at all.
