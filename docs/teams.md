...
**The base and its consumer shipped together**, because a subsystem with no consumer and a constraint with
...
## `team_members` needed a writer and nothing else, checked rather than assumed
...
| list, with sizes | `GET /api/teams` | any member | not audited |
| read one, with its roster | `GET /api/teams/:id` | `org.admin` | not audited |
...
nothing, the argument that lets any member open a matter (`src/matters.ts`). It is `org.admin` anyway, and
the reason is about the *name* rather than the authority. Names are unique in an organization, so creating a
team takes a name out of a shared space other people's grants will be chosen from, and `createPolicyDraft`
already requires `org.admin` for exactly that. A team anybody could create is a team an administrator might
later grant `mailbox.content.read` to, believing it to be the one their colleague described.
...
- **A UI.** The shell is Layer 1 to 3's surface; the policy, approval and supervised planes have none either.
