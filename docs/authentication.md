...
the state-changing endpoints CSRF-safe without a separate token", and that was false** (#96). Lax does
...
- **Length only, no character classes.** Minimum 12. Composition rules produce `Password1!`;
  ...
  so it is recorded as absent rather than pretended at.
...
- `recovery.vault_restored` is written **knowing**, `ok` when it ran to the end and `failed` when the vault
