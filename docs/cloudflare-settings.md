...
- **One row cannot be done from Mailda**, the OAuth client, and the reason is measured rather than assumed:
  no API exists for a private client. The handover manifest `mailda provider --handover` prints names it as
  *what a person must still do at the provider*, with the receipt that established it. The `email.sending`
