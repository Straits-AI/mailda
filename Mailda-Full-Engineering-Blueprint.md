...
One-click and CLI paths must converge on equivalent resource inventory, migration state and `doctor` results. The repository reproduces the **code**; configuration is reproduced from the §24 backup manifest, not from git (ADR 24). The installer offers a delegated operational subdomain (the default) or a root-domain cutover, and refuses to imply that two independent root MX authorities can coexist deterministically.
