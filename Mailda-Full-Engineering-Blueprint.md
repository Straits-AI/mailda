...
**Not built, and additive rather than blocked:** passkeys (§5A step 2), OIDC/SAML federation, SCIM,
step-up authentication, DPoP. **Password authentication is the weakest link in this design and is a
deliberate interim** — the receipt states plainly why Workers leaves no better primitive available.
...
28. **The credential KEK is the one binding that is not account-portable, and ADR 24's guarantee does not currently cover it.** Added 4 August 2026, unresolved and recorded rather than papered over. ADR 22 requires credentials to be Secrets Store bindings read via `await env.NAME.get()`, and ADR 24 requires the Node's repository to be byte-identical across every installation. These collide: Cloudflare links D1 and R2 per-account with no id in config, but a `secrets_store_secrets` binding needs an account-specific `store_id` **in committed configuration**. So `wrangler.jsonc` currently carries a development account's store id — precisely the customer-specific value ADR 24 exists to eliminate. Neither available resolution is free: having the installer write the binding means the Deploy-to-Cloudflare path cannot provision it, and moving the KEK somewhere per-account linking reaches means giving up Secrets Store's async accessor, which is the property that keeps the value off `env` and out of logs. The decision is deferred deliberately; the alternative was to commit the id quietly and let a later installation discover it. A related, smaller finding: a Secrets Store secret is `pending` immediately after creation and `.get()` can fail until it propagates, which surfaced as a first-sign-in failure and argues for `doctor` checking readability rather than presence.
