...
import {
  ...
  type DeliveryRow, type DoctorReport, type ProviderBinding, type RoutingRow,
} from "./api.ts";
...
export function onboardingSteps(sources: Sources): Step[] {
  ...
  const routed = routing === undefined
  ...
  const sending = delivery === undefined
    ? notAsked(connected.state === "done" ? "not read" : "needs the connection first")
    : delivery === null
  ...
  const outcomes = delivery === undefined
    ? notAsked(connected.state === "done" ? "not read" : "needs the connection first")
    : delivery === null
      ? notAsked("delivery events could not be read through the grant")
...
const CHIP: Record<StepState, string> = { done: "verdict-ok", todo: "severity-degraded", unknown: "state-outcome_unknown" };
const WORD: Record<StepState, string> = { done: "done", todo: "to do", unknown: "unknown" };

/** The five steps and a count, at the top of Setup. Reads the grant-spending sources only once connected. */
export function OnboardingProgress({ binding }: { binding: ProviderBinding }) {
  ...
  return (
    <section className="onboarding" aria-label="Setup progress">
      <p className="dim">
        <span className="mono num">{done}</span> of <span className="mono num">{steps.length}</span> steps done
        {done === steps.length ? "." : `; next: ${steps.find((step) => step.state !== "done")!.label.toLowerCase()}.`}
      </p>
      <ol>
        {steps.map((step) => (
          <li key={step.id}>
 ...
 * connection state and `doctor`, both already fetched for the rail and the instrument bar, and never the
 * grant-spending sources: a glance at the inbox must not renew a token.
 */
export function SetupUnfinished() {
  ...
  const undone = onboardingSteps({
    provider: provider.data.provider,
    doctor: doctor.isSuccess ? doctor.data : null,
  }).find((step) => (step.id === "connected" || step.id === "address") && step.state === "todo");
  if (undone === undefined) return null;
  return (
    <p className="notice" role="status">
      Setup is unfinished: {undone.label.toLowerCase()} ({undone.detail}).{" "}
