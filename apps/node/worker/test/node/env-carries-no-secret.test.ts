...
describe("no credential is a plain property of env (ADR 22)", () => {
  ...
  it("gives every member a binding type, and never a primitive", () => {
    /*
     * **The allowlist is consulted by name, never by type**, and that distinction is the guard.
     *
     * This scan's own message offers `BINDING_TYPES` as the escape hatch — *"if this really is innocuous,
     * add it to BINDING_TYPES"* — and taking it would mean allowing `string`, which retires the check
     * entirely, because every future secret is a string too. So an argued **member** is skipped by name and
     * every other primitive still fails.
     *
     * Found by CI rather than here, and that gap is now closed. `worker-configuration.d.ts` is generated by
     * `wrangler types` and is **gitignored**, so CI regenerates it from a clean checkout while a local run
     * read whatever was last written. The half of this file that reads `wrangler.jsonc` caught the new var;
     * this half, which reads the generated env, did not — and a red PR was the first anybody knew.
     *
     * `pnpm test` runs `wrangler types` first now. A check that reads a generated file is only as current as
     * that file, and one that can pass while wrong is the thing this repository is built to avoid.
     */
    const offenders = envMembers().filter(
      ([name, type]) => !(name in ALLOWED_VARS)
        && !BINDING_TYPES.some((allowed) => type.startsWith(allowed)),
    );
    expect(
  ...
  it("rejects a var that is not on the list, so the allowlist is not decoration", () => {
    ...
    /*
     * And the type scan still rejects a primitive nobody argued for — which allowing `string` in
     * `BINDING_TYPES` would have destroyed. `WORKER_NAME` passes on its name, never on its shape.
     */
    const wouldReject = (name: string, type: string) =>
      !(name in ALLOWED_VARS) && !BINDING_TYPES.some((allowed) => type.startsWith(allowed));
    expect(wouldReject("MAILDA_API_TOKEN", '"a-secret"')).toBe(true);
    expect(wouldReject("WORKER_NAME", '"mailda"')).toBe(false);
  });
});
