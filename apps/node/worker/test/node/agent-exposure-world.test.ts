...
describe("every route is classified, and a new one cannot default", () => {
  ...
  it("derives read for every GET but the named exceptions, and there are eighteen", () => {
     ...
     * on.
     *
     * `GET /api/provider/sending` (#163) is the thirteenth, and it is the first here that is withheld for
     ...
     *
     * `GET /api/provider/handover` (#165) is the fifteenth. It contains the ownership read, so it inherits
     ...
     *
     * The seventeenth and eighteenth are the purchase's two reads. `…/domains/purchase` carries the
    ...
    expect(exceptions.sort()).toEqual([
      ...
      "GET /api/provider/receiving", "GET /api/provider/sending", "GET /api/provider/subscription",
